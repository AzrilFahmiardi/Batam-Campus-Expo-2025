      ...
      try {
        ...
        if (rows.length > 0) {
          ...
          const [result] = await db.query(
            "INSERT INTO user (username, email,has_voted) VALUES (?, ?,?)",
            [username, email, has_voted]
          );
...
app.post("/vote", async (req, res) => {
  ...
  try {
    ...
    const [user] = await db.query(
      "SELECT has_voted FROM user WHERE email = ?",
      [req.user.email]
    );
    ...
    await connection.query("UPDATE user SET has_voted = 1 WHERE email = ?", [
      req.user.email,
    ]);
...
app.get("/:kode_univ", async (req, res) => {
  ...
  try {
    const [hasil_univ] = await db.query(
      "SELECT * FROM universitas WHERE kode_univ = ?",
      [kode_univ]
    );
...
app.get("/:kode_univ/jurusan", async (req, res) => {
  ...
  try {
    const [jurusan] = await db.query(
      "SELECT * FROM jurusan WHERE kode_univ = ?",
      [kode_univ]
    );
    if (jurusan.length > 0) {
      ...
      res
        .status(404)
        .json({ message: "Jurusan tidak ditemukan untuk universitas ini" });
    }
  } catch (error) {
